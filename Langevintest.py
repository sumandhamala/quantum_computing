"""Created on Thu Jul 12 14:56:30 2018@author: Suman Dhamala
...
def potenergy(X,phixx):
    return const2*( 0.5*( X- phixx )**2  - const* np.cos(X) )
...
### feedback process has two fast ramp part
    ...
    Network[i] = -const3*ej*cumsumWORK[-1]
    #print("Network", Network[i])
#    print("count", count)
#    NETWORK.append(Network[i])
#    NW = np.vstack(NETWORK)
#    NWA = [-const3*ej*cumsumWORK[-1] for i in range(0,rows)]  
##   
...
############################calculate heat and work############################
    for ii in range(0,len(state)-1):
        if state[ii+1]== state[ii]:
            dizWork[ii+1]= dizWork[ii]+ 0.5*state[ii+1]*(E[ii+1]-E[ii])
            Qheat[ii+1]= Qheat[ii]
        else:
            dizWork[ii+1]= dizWork[ii]
            Qheat[ii+1]= Qheat[ii] - 0.5*( state[ii+1]-state[ii] )*( E[ii+1]+E[ii] )*0.5   
#################################################### #############################          
    ## saving the results ##########
    finalresult[i] = [ dizWork[-1], Qheat[-1] , Network[i]] 
    np.savetxt('finalresult.txt', finalresult, fmt="%.4f", delimiter="\t")
...
#plt.savefig('../Images/Fig_ex7_RKF45.png', dpi=600)
