...
##
    while Phi_x[count]  <  Phi_x[round(feedback+1 )]:
     ...
     Work[count] = potenergy( phi[count-1], Phi_x[count-1]) - potenergy( phi[count], Phi_x[count])
    ...
    if phi[count] >  position[1]:
            ...
            while Phi_x1[count]  > endpoint1:
             ...
             ## loop to find position of the particle   
                ...
                ## work done by the engine
                Work[count] = potenergy( phi[count-1], Phi_x1[count-1]) - potenergy( phi[count], Phi_x1[count])
...
## initialized the count to run the main for loop
count=0
###############################################################################
